...
uint32_t HRFCmdParser::parse_u32(char* s)
{
...
uint64_t HRFCmdParser::parse_u64(char* s)
{
	uint_fast8_t base = 10;
	char* s_end;
	uint64_t u64_value;
...
int64_t HRFCmdParser::parse_frequency_i64(char* optarg, char* endptr)
{
	if (optarg == endptr) {
		throw SDRException();// return HACKRF_ERROR_INVALID_PARAM;
	}
...
uint32_t HRFCmdParser::parse_frequency_u32(char* optarg, char* endptr)
{
	if (optarg == endptr) {
		throw SDRException();// return HACKRF_ERROR_INVALID_PARAM;
	}
...
char* HRFCmdParser::u64toa(uint64_t val, HRFUtil::t_u64toa* str)
{
...
char* HRFCmdParser::stringrev(char* str)
{
	char* p1, * p2;
